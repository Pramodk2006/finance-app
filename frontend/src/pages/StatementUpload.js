...
const DropzoneBox = styled(Box, {
  shouldForwardProp: (prop) => !['isDragActive', 'isDragReject', 'isSuccess'].includes(prop)
...
const StatementUpload = () => {
  ...
  const onDrop = useCallback(async (acceptedFiles) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Failed to upload statement. Please make sure the backend server is running and you are logged in.');
    } finally {
      setUploading(false);
    }
            ...
            {uploading ? (
              ...
              <Box display="flex" flexDirection="column" alignItems="center">
                ...
                <Typography variant="body1">
                  {results?.count} transactions imported
                  {results?.duplicates > 0 ? `, ${results.duplicates} duplicates skipped` : ''}
                </Typography>
              </Box>
            ) : (
              <Box display="flex" flexDirection="column" alignItems="center">
                <CloudUploadIcon style={{ fontSize: 50, color: '#bdbdbd' }} />
                <Typography variant="h6" mt={2}>
                ...
                <Typography variant="caption" color="textSecondary" mt={1}>
                  Supports CSV, PDF, and image formats (JPG, PNG)
                </Typography>
              </Box>
            )}
          </DropzoneBox>
      ...
      <Card sx={{ mt: 4 }}>
        <CardContent>
          ...
          <Typography variant="body2" color="primary" sx={{ mt: 2 }}>
            <strong>Pro Tip:</strong> For screenshots, make sure the image is clear and text is readable. Tables with dates, 
            descriptions, and amounts work best.
          </Typography>
        </CardContent>
      </Card>
    </UploadContainer>

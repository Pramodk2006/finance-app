  ...
  return new Promise((resolve, reject) => {
    fs.createReadStream(filePath)
      ...
      .on("data", (data) => {
        ...
        const dateField =
          data[dateColumn] || data["Transaction Date"] || data["Date"] || "";
...
const parseImage = async (filePath, options = {}) => {
  try {
    ...
    for (const transaction of transactions) {
      const key = `${transaction.date.toISOString()}_${transaction.amount.toFixed(
        2
      )}_${transaction.description.substring(0, 20)}`;
      if (!seen.has(key)) {
        seen.add(key);
  ...
  try {
    ...
    switch (ext) {
      ...
      default:
        ...
        throw new Error(
          `Unsupported file type: ${ext}. Only CSV, PDF, and image files are supported.`
        );
    }

    console.log(
      `Successfully parsed ${transactions.length} transactions from ${filePath}`
    );
    ...
    const validTransactions = transactions.filter((tx) => {
      ...
      if (!tx.date || !(tx.date instanceof Date) || isNaN(tx.date.getTime())) {
        console.warn(
          `Skipping transaction with invalid date: ${JSON.stringify(tx)}`
        );
        return false;
      }
      ...
      if (isNaN(tx.amount) || tx.amount <= 0) {
        console.warn(
          `Skipping transaction with invalid amount: ${JSON.stringify(tx)}`
        );
        return false;
      }
      ...
      if (!tx.description || typeof tx.description !== "string") {
        console.warn(
          `Skipping transaction with invalid description: ${JSON.stringify(tx)}`
        );
        return false;
      }
